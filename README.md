...
![Redis](https://img.shields.io/badge/Redis-optional-DC382D?style=flat-square)
...
本仓库基于开源项目 [itning/yunshu-nas](https://github.com/itning/yunshu-nas) 二次开发，遵循 Apache-2.0。
**包名（`top.itning.yunshunas.*`）与原作者署名全部保留**，原项目版权归原作者所有（见 [LICENSE](LICENSE)）。
...
| 范围 | 说明 |
|---|---|
| **后端** | 并发治理、SQL 与接口、中间件接入、缺陷修复 |
| **前端** | 仅播放页（为接通转码产物），其余交互与样式未改动 |

每项改动对应一次提交，message 按「问题 → 改动 → 实测」三段写：改动前建基线、改动后复测。
**没有测量的改动不列入下面的成效表。** 完整路线图见 [`docs/optimization-plan.md`](docs/optimization-plan.md)。
...
表中「音乐列表接口」只列响应体积：体积三次测量完全一致（17,932,552 字节），
而耗时受 JIT 与连接池预热影响很大（同一接口冷启动 196 ms、预热后 14 ms），不同状态的值不具可比性。
复现方法见文末「怎么复现这些数字」。

> 表中最后两行来自同一次改动（清理永不生效的容器依赖），属于顺带产生的影响而非独立的优化目标。
> 其中**启动耗时受机器负载影响较大**，建议只作参考。
...
1. `VideoTransformHandler` 被注入到 Controller 后**从未被调用**，`put()` / `status()` 全仓零调用者；
   前端播放走原始文件直链、**从不请求 m3u8** —— 整条链路不可达。
2. 切片命令用的 `-vbsf` 选项**已被新版 ffmpeg 移除**（代码注释中标注的是 ffmpeg 4.1.3），
   执行时退出码 8、无产物；而 `CommandUtils` **不检查退出码**，失败被当作成功上报（任务计数照常 +1）。
3. 再叠加"m3u8 文件存在即已完成"的判据 —— 产物永远不存在，同一文件可以无限重复提交、每次失败、每次报成功。

**处理方式**：接通触发入口 → 修正切片命令 → 让失败可见（`failedCount` + ERROR 堆栈）→
清理残缺产物（使失败后可重转）→ 修正子进程治理（超时 / 退出码 / 强制回收）→ 接通前端播放
（播放页可在原文件与转码版本之间切换，一键转码后自动切换）。

> 后端修好不等于功能可用：产物没有消费方时，用户视角下 mkv 依然打不开。这一步是后来补的。

### 2. 容器替换：评估确认在当前版本上无法实现

上游 `pom.xml` 排除了 Tomcat 并引入 Undertow，意图是换容器。实测发现这是三层问题叠加：

1. exclusion **打在错误的 artifact 上** —— Tomcat 是从 `spring-boot-starter-websocket`
   → `spring-boot-starter-webmvc` → `spring-boot-starter-tomcat` 进来的（Boot 4.x 新增了 webmvc 中间层）；
2. 补上 exclusion 让 Undertow 生效后，应用**启动失败** —— 上游钉的 `4.0.0-M1`
   是 Boot 4.0 线的里程碑版本，与 4.1.1 的 `PropertyMapper` API 不兼容；
3. 根因是 **Boot 4.x 已不再支持 Undertow**：4.1.1 的 BOM 只管理 tomcat / jetty / netty，
   该 starter 在 Maven Central 的版本止于 `4.0.0-M1`。

**处理方式**：不在这个方向上继续投入，而是清理掉永不生效的依赖（省 4.55 MB），
并把原本只对 Undertow 生效（因此从未运行过）的 WebSocket 定制器改为对当前容器生效的写法。

### 3. 配置变更：从仅本机生效扩展到多实例

项目里已有 `ConfigChangeEvent` + `ApplicationListener` 机制 —— `FtpConfig`、`ElasticsearchConfig`、
`DataSourceConfig` 都监听它来重建自身。**该机制原本只在本机生效**：在实例 A 改配置，实例 B 无感知。

**处理方式**：用 Redis 发布/订阅把变更广播到所有实例，收到远端变更的实例在本机落库并触发同样的组件重建。
...
### 4. 一个反复出现的缺陷模式：初始化阶段做网络 IO 却没有失败边界

排查过程中发现同一类问题出现了两次，都记录在此。

**Redis**：连接自检包了 `try/catch`，紧随其后的订阅启动却没有；而 `RedisMessageListenerContainer.start()`
会**同步**取一次连接，不可达时直接抛异常。异常穿出 `@PostConstruct` 后 Spring 取消整个刷新，
结果是**应用连首页都打不开** —— 不是"Redis 功能不可用"。这个分支读代码看不出来，
是实测"先配好 Redis、再把 Redis 关掉"才暴露的。

**Elasticsearch**：`ElasticsearchConfig.init()` 在做完配置后**立刻发网络请求建索引**，同样没有失败边界。
后果更难处理 —— 在设置页勾选「开启 Elasticsearch」而 ES 未运行，保存当场返回 500（**配置却已落库**），
之后每次启动都 `Application run failed`；而**设置界面本身由该应用提供**，只能删除 `yunshu-nas.db`
（连同 MySQL、ffmpeg 的设置一起丢）。

两处都已按「失败只 WARN + 彻底降级（`enabled()==false`，调用点全走 no-op）」修复。
关键是要降得**彻底**：只记日志不清模板时，后续每次调用都会抛异常，
...
- **视频点播** —— 文件浏览 / 转码为 HLS / 在线播放
- **音频点播** —— 增删改查 / 歌词 / 封面 / 搜索，支持 WebDAV（`path:/webdav`）
- **远程下载** —— 基于 aria2c，可不启用
- **FTP 服务**
...
| 层次 | 技术 | 用途 |
|---|---|---|
| 语言 / 运行时 | Java 21 | 应用运行环境 |
| 框架 | Spring Boot 4.1.1（内嵌 Tomcat）、Spring JDBC（JdbcTemplate） | Web 与数据访问 |
| 数据库 | MySQL 8 / SQLite（可选）、Elasticsearch 9（可选） | 业务数据 / 内嵌设置 / 歌词检索 |
| 缓存与协调 | Redis（可选） | 分布式锁、配置广播 |
| 消息队列 | RabbitMQ（可选，默认关闭） | 转码任务队列 |
| 其他 | HikariCP、Caffeine、ffmpeg、Angular 22 + ng-zorro | 连接池 / 本地缓存 / 音视频处理 / 前端 |

> **"可选"是真的可选**：MySQL / ES / Redis / RabbitMQ 全都可以不配。
> 应用自身的设置存在内嵌 SQLite 中，**零配置即可启动** —— 这是本项目一直刻意保持的特性。
...
按主题分组。每条的**问题 → 改动 → 实测**写在对应提交的 message 里。
...
| 改动 | 说明 |
|---|---|
| 删除冗余索引 | `music_id` 上同时有唯一索引与普通索引；删除前后执行计划**逐列完全一致**，说明优化器未使用过它 |
...
| 新增分页接口 | 列表接口原本一次返回全表；**新增** `/api/music/page`（不改老接口契约），每页条数设上限 |
...
后端修好后仍需解决消费方缺失：前端播放器一直播原始文件直链（`/video/{path}`），
从不请求 m3u8 —— 即使转码成功，mkv / avi 这些浏览器放不了的格式用户依然打不开。

| 层 | 改动 |
|---|---|
| 后端 | 新增 `GET /transcode/info?location=`：只读返回产物是否就绪、播放地址、是否正在转码。**刻意不调用 `getWriteDir()`** —— 那个方法会顺手 `mkdirs`，查询接口不应有副作用 |
| 前端 | 播放页加"原文件 / 转码版本 / 转码后播放"三个控件；原文件播放失败时给出提示；提交转码后轮询，就绪即自动切换 |
| 体积 | `hls.js` 用**动态 import**：压缩后约 586 KB，列表页与音乐页用不到，打进主包等于让所有人先下载。实测主包只 +4.5 KB，hls 独立成 chunk |
| 离线可用 | 不用 dplayer 内置的 `type:'hls'` —— 它会在运行时从 **CDN** 拉 hls.js，而 NAS 常是内网离线环境 |
...
| 约 5 秒后 | 自动切到转码版本，`video.src` 变成 `blob:` 地址 —— 这是 hls.js 走 MSE 播放的标志（播原文件会是 `http://…/video/…`） |
| 起播验证 | `currentTime` 由 0 推进到 2.96、`paused=false`、分辨率 `1920×1080`、时长 60.2 s、`error=null` |

**一个边界**：`canPlay` 按**后缀名**判断，而浏览器能不能播取决于**容器 + 编码**，两者不等价。
实测撞到反例 —— 同一个 H.264/AAC 的 `.mkv`，Chrome 能直接播（列表页标 `canPlay=true` 是对的）；
而 `.avi` 同样标了 `canPlay=true` 却放不了（`error.code=4`）。
失败提示存在的理由就是这个：**后缀判断不准，所以用播放器的 `error` 事件兜底，而不是在前端再维护一份白名单。**
...
| 项 | 结论 |
|---|---|
| 前导通配符 `LIKE '%kw%'` | **加索引也救不了**。实测同一列同一个索引：`LIKE 'Chou%'` 走 `range`、`LIKE '%Chou%'` 仍是全表扫描。这类查询应交给 ES 倒排索引。详见 [`docs/analysis/fuzzy-search-and-index.md`](docs/analysis/fuzzy-search-and-index.md) |
| 深分页 | 当前是 `OFFSET` 分页，页号越大越慢（实测第 100 页是首页的约 3.5 倍）。彻底解决需改游标（keyset）分页 |
| 列表逐行 URI 调用 | **实测证否后放弃**：分页后每页仅 100–200 行，整页 14 ms，没有可优化的瓶颈 |
| 删歌流程加事务 | **读码证否后放弃**：该流程 5 步里只有 1 条 DB 语句，单条语句本身即原子，`@Transactional` 提供不了额外原子性；文件与 ES 也参与不了 JDBC 事务 |
...
| Elasticsearch 的两个功能 | **界面上不可达**：前端从不调用 `/music/search_v2`（歌词全文搜索）与 `/api/music/reInitLyric`（重建索引）。用户能触达 ES 的只有"删歌"与"上传歌词"两条被动写入；列表页搜索框走的是数据库 `LIKE`，与 ES 无关。修完启动问题后这个不对称仍在 |
...
| 转码的触发方式 | **半自动**：由用户点一次触发转码，没有做成"发现放不了就静默转码" |
| 正式压测 | 只有单请求延迟基线，**未做多线程 QPS 曲线** |

## 界面

界面来自原项目（Angular 22 + ng-zorro）。本次改造只动了**播放页**：增加"原文件 / 转码版本 / 转码后播放"
三个控件与一段失败提示，用于接通转码产物。其余页面的交互与样式未改动。
...
1. **单次 `EXPLAIN ANALYZE` 受缓冲池冷热影响很大** —— 同一个查询先后见过 58 ms 与 20 ms。
   结论应建立在"同一条件下各跑多次取中位数"上。验证联合索引时是把索引先摘掉跑 5 次、
   再加回来跑 5 次，避免拿不同时刻的单次值互相比较。
