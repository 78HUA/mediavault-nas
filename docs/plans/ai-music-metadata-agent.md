...
1. **构造测试集**：手工整理 **50–100 条**已知正确答案的样本
   ...
   - 可选捷径：如果 `.baseline/02_seed_music.sql` 还在，
     那份灌库脚本用**确定性公式**生成 5 万行，所以"正确答案"是可计算的 ——
     把 `name`/`singer`/`type` 打乱成假文件名就成了天然的测试集。
     如果该文件已不存在，用上面的手工方式即可
2. **统计四个指标**，写进文档：
...
1. **`metaInfo()` 可能返回 `null`**（文件无标签时）。Agent 的"读标签"工具必须能表达"读到了但没有"，
   而不是抛异常 —— 这个区分对模型的决策很重要（读到空标签才需要去查库）
